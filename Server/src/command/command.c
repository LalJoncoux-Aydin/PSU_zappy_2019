...
void msz(int fd_cli, client_t *clis __attribute__((unused)),
server_t *server, char *command __attribute__((unused)))
{
    char *buff = malloc(20);
...
void bct(int fd_cli, client_t *clis __attribute__((unused)),
server_t *server, char *command)
{
    ...
    , x, y, tile->q0, tile->q1, tile->q2, tile->q3, tile->q4,
    tile->q5, tile->q6);
    send(fd_cli, buff, strlen(buff), 0);
    if (DEBUG)
        printf("message send : %s", buff);
...
void mct(int fd_cli, client_t *clis __attribute__((unused)),
server_t *server, char *command  __attribute__((unused)))
{
    char *buff = malloc(50);
    char *res = malloc(50);
    ...
    for (int y = 0; y < server->y; y++)
        for (int x = 0; x < server->x; x++) {
            tile = &(server->map[y][x]);
            sprintf(buff, "bct %d %d %d %d %d %d %d %d %d\n"
            , x, y, tile->q0, tile->q1, tile->q2, tile->q3,
...
void tna(int fd_cli, client_t *none __attribute__((unused)),
server_t *server, char *none_c  __attribute__((unused)))
{
    char *buff = malloc(50);
...
void pnw(client_t *cli)
{
    ...
    cli->ai->x, cli->ai->y, cli->ai->orientation,
    cli->ai->level, cli->ai->team);
    if (DEBUG)
        printf("%s", buff);
