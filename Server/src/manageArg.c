...
static bool manageOther(char op, char *opt_arg, server_t *server, char *limit)
{
    int i = 0;
    char *buff_name = NULL;
...
static void manageNb(char op, char *opt_arg, server_t *server)
{
    switch (op) {
        case 'x':
...
void manageArg(int ac, char **av, server_t *server_v, char **env)
{
    ...
    while (op != -1) {
        ...
        if (op == -1)
            break;
        if (manageOther(op, optarg, server_v, env[0]) == false)
            error("Error : Argument wrong");
        manageNb(op, optarg, server_v);
    }
}
